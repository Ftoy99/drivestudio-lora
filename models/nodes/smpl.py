...
from models.human_body import phalp_colors, SMPLTemplate, get_on_mesh_init_geo_values, batch_rigid_transform, \
...
class SMPLNodes(RigidNodes):
    ...
    def transform_means(self, means: torch.Tensor) -> torch.Tensor:
        ...
        if self.in_test_set and (
            ...
            _prev_masked_theta = \
            torch.cat((self.instances_quats[self.cur_frame - 1], self.smpl_qauts[self.cur_frame - 1]), dim=1)[
                instance_mask]
            _next_masked_theta = \
            torch.cat((self.instances_quats[self.cur_frame + 1], self.smpl_qauts[self.cur_frame + 1]), dim=1)[
                instance_mask]
            _cur_masked_theta = \
            torch.cat((self.instances_quats[self.cur_frame], self.smpl_qauts[self.cur_frame]), dim=1)[instance_mask]
            interpolated_theta = interpolate_quats(_prev_masked_theta, _next_masked_theta)

            inter_valid_mask = self.instances_fv[self.cur_frame - 1, instance_mask] & self.instances_fv[
                self.cur_frame + 1, instance_mask]
        ...
        W, A = self.template(
            masked_theta=masked_theta,
            instances_mask=instance_mask,
            xyz_canonical=means.reshape(self.num_instances, self.smpl_points_num,
                                        3) if self.use_voxel_deformer else None
        )
        T = torch.einsum("bnj, bjrc -> bnrc", W, A)
        R = T[:, :, :3, :3]  # [N, 3, 3]
        t = T[:, :, :3, 3]  # [N, 3]
    ...
    def transform_means_and_quats(self, means: torch.Tensor, quats: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        quats_container.index_add_(0, (~instance_mask).nonzero().squeeze(),
                                   torch.tensor([[[1., 0., 0., 0.]]], device=self.device).repeat((~instance_mask).sum(),
                                                                                                 6890, 1))
    ...
    def compute_reg_loss(self):
        ...
        if knn_reg is not None:
            ...
            if not self.ctrl_cfg.freeze_shs_dc:
                valid_shs_dc = self._features_dc.reshape(self.num_instances, self.smpl_points_num, 3)[
                    instances_mask]  # (num_instances, smpl_points_num, 3)
                nn_ind_expanded = nn_ind.unsqueeze(-1).expand(-1, -1, -1, 3)
                knn_shs_dc = torch.gather(valid_shs_dc.unsqueeze(2).expand(-1, -1, K, -1), 1,
                                          nn_ind_expanded)  # (num_instances, smpl_points_num, knn_neighbors, 3)
            ...
            if not self.ctrl_cfg.freeze_shs_rest and self.sh_degree > 0:
                dim_sh = num_sh_bases(self.sh_degree)
                valid_shs_rest = self._features_rest.reshape(self.num_instances, self.smpl_points_num, -1)[
                    instances_mask]  # (num_instances, smpl_points_num, (dim_sh-1)*3)
                nn_ind_expanded = nn_ind.unsqueeze(-1).expand(-1, -1, -1, (dim_sh - 1) * 3)
                knn_shs_rest = torch.gather(valid_shs_rest.unsqueeze(2).expand(-1, -1, K, -1), 1,
                                            nn_ind_expanded)  # (num_instances, smpl_points_num, knn_neighbors, (dim_sh-1)*3)
            ...
            if not self.ctrl_cfg.freeze_o:
                valid_o = self.get_opacity.reshape(self.num_instances, self.smpl_points_num, 1)[
                    instances_mask]  # (num_instances, smpl_points_num, 1)
            ...
            if not self.ctrl_cfg.freeze_s:
                ...
                valid_s = self.get_scaling.reshape(self.num_instances, self.smpl_points_num, scale_dim)[
                    instances_mask]  # (num_instances, smpl_points_num, 1)
            ...
            if not self.ctrl_cfg.freeze_q:
                valid_q = self._quats.reshape(self.num_instances, self.smpl_points_num, 4)[
                    instances_mask]  # (num_instances, smpl_points_num, 4)
    ...
    def state_dict(self) -> Dict:
        ...
        state_dict.update({
            ...
            "instances_quats": self.instances_quats
        })
        return state_dict
    ...
    def load_state_dict(self, state_dict: Dict, **kwargs) -> str:
        ...
        print("debug intializing_quats for smpl")
    ...
    def get_instance_activated_gs_dict(self, ins_id: int) -> Dict[str, torch.Tensor]:
        ...
        activated_scales = torch.exp(
            self._scales[pts_mask].repeat(1, 3) if self.ball_gaussians else self._scales[pts_mask])
    ...
    def deform_gaussian_points(
        ...
        W, A = self.template(
            masked_theta=self.quat_act(masked_theta),
            instances_mask=instance_mask,
            xyz_canonical=means.reshape(1, self.smpl_points_num, 3).repeat(self.num_instances, 1,
                                                                           1) if self.use_voxel_deformer else None
        )
        T = torch.einsum("bnj, bjrc -> bnrc", W, A)
